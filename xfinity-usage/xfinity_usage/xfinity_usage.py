...
class XfinityUsage(object):
    ...
    # set browser type
    def __init__(self, username, password, debug, attempts, cookie_file='cookies.json', browser_name='firefox-headless'):
        """
        Initialize class.
        ...
        logging.info('XfinityUsage loaded with the following variables:')
        logging.info('username: ' + self.username)
        logging.info('password: ' + self.password)
        logging.info('debug: ' + str(debug))
        logging.info('attempts: ' + str(self.attempts))
        logging.info('cookie_file: ' + self.cookie_file)
        logging.info('browser_name: ' + self.browser_name)
    ...
    def enter_username_and_submit(self):
        logger.debug('Entering Username Text')
        try:
            ...
            raise RuntimeError("Unable to find username input box!")

        logger.debug('Getting Sign In button')
        try:
    ...
    def do_login(self):
        ...
        self.enter_username_and_submit()
        
        logger.debug('Entering Password Text')
        try:
            ...
            self.error_screenshot()
            raise RuntimeError("Unable to find passwd input box!")
        
        logger.debug('Checking remember me box')
        try:
            self.do_screenshot()
            rem_me = self.browser.find_element_by_id('remember_me')
            if not rem_me.is_selected():
                logger.debug('Clicking "Remember Me"')
                # because of layering issues, for chrome-headless we need to click the containing span instead of the checkbox itself
                self.browser.find_element_by_id('remember_me_checkbox').click()
        except Exception:
            self.error_screenshot()
        ...
        logger.debug('Checking for page redirect')
        count = 0
    ...
    def get_browser(self):
        ...
        if 'firefox' in self.browser_name:
            ...
            if self.browser_name == 'chrome-headless':
